...
const albumAuthorization = async (req, res, next) => {
  try {
    ...
    if (!isAllowed) {
      ...
      return next(
        new Unauthorized(
          `Vous n'avez pas les droits pour modidier cette ressouce`
        )
      )
    }
    ...
    errorLogger('Album Authorisation Error', error)

    return next(error)
  }
}

module.exports = albumAuthorization
