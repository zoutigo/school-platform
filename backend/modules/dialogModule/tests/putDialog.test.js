...
describe('DIALOG - PUT', () => {
  ...
  it('should create new dialog', async () => {
    ...
    const toUpdateDialog = {
      title: 'un nouveau dialogue',
      content: faker.lorem.sentence(2),
      startdate: moment().add(2, 'days').toDate().toISOString(),
      enddate: moment().add(8, 'days').toDate().toISOString(),
    }

    const updatedDialog = await request(app)
      .put(`/api/dialogs/${newDialog.body.datas.uuid}`)
      .set(managerTokenHeader)
      .send(toUpdateDialog)

    expect(updatedDialog.statusCode).toEqual(200)

    expect(updatedDialog.statusCode).toEqual(200)
    expect(updatedDialog.body.datas).not.toHaveProperty('id')
    expect(updatedDialog.body.datas).toHaveProperty('uuid')
    expect(updatedDialog.body.datas).toHaveProperty('title')
    expect(updatedDialog.body.datas).toHaveProperty('content')
    expect(updatedDialog.body.datas).toHaveProperty('startdate')
    expect(updatedDialog.body.datas).toHaveProperty('enddate')
    expect(updatedDialog.body.datas).toHaveProperty('createdAt')
    expect(updatedDialog.body.datas).toHaveProperty('updatedAt')
    expect(updatedDialog.body.datas).toHaveProperty('deletedAt')
    expect(updatedDialog.body.datas.title).toEqual(toUpdateDialog.title)
  })
})
